...
- **Optimizer:** Three-phase — greedy assignment (with a post-greedy **Stage-4 feasibility-split** pass — see the slot-level shift-splitting design rule), simulated-annealing local search (pairwise swap + insert-into-unfilled), and a deterministic post-SA polish (`polishReplaceWithIdle`) that scans (assigned-incumbent, idle-eligible) pairs and accepts strict composite-score improvements. Polish runs inside `localSearchOptimize` after the existing post-SA insert sweep, skips `sameGroupRequired` tasks (within-group is SA's job, cross-group is HC-4-infeasible) and pinned/Manual/Frozen incumbents, reuses `IncrementalScorer` for O(k) per-attempt scoring, and is capped at 3 passes. Multi-attempt runs pick the best result.
...
- **Shift-splitting:** Slot-level feasibility recovery — an unfillable slot of a `splittable` occurrence is realized as two half-tasks covered by two different people, leaving the occurrence's other slots whole on a residual task. Generation-only (greedy Stage-4); HC-16 + the link-union enforce correctness. Full invariants in the slot-level shift-splitting design rule above.
