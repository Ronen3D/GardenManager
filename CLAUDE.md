...
Garden Manager is a constraint-based scheduling system for garden teams. It generates weekly schedules for participants across 7 task types (Adanit, Hamama, Shemesh, Mamtera, Karov, Karovit, Aruga), respecting hard constraints (eligibility, certification, availability) and optimizing soft constraints (rest fairness, workload balance). The UI is in Hebrew.
...
Constraints (src/constraints/)
  ...
  └── senior-policy.ts     — HC-13: L2/L3/L4 natural role isolation
        │
Foundation
  ├── models/types.ts          — All enums, interfaces, type aliases
  ├── tasks/task-definitions.ts — Factory functions for each task type
...
- **Certifications:** Nitzan, Salsala, Hamama, Horesh — gate eligibility for specific task types.
