...
const PARTICIPANTS_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      placement: 'top',
      title: 'העדפות משימה',
      body: 'בעריכת משתתף תוכל להגדיר משימה <strong>מועדפת</strong> ו<strong>פחות-מועדפת</strong>. אלו אילוצים רכים — האופטימייזר יעדיף לכבד אותם, אך אינו מחויב לכך. אם תבחר משימה שהמשתתף לא כשיר אליה (רמה / הסמכה לא מתאימה), תופיע אזהרה מתחת לשדה — ההעדפה תישמר אך לא תיקח חלק בפועל.',
    },
    {
      id: 'p-7b',
      ...
      body: 'לחץ <strong>שלוש פעמים</strong> על מספר המשתתפים כאן בכותרת כדי לחשוף עמודת <strong>אי-התאמה</strong>. בשורת עריכת כל משתתף תופיע שדה לרישום שמות משתתפים שעדיף לא לשבץ אליו יחד (מופרדים בפסיק). זהו <strong>אילוץ רך</strong> — האופטימייזר ישתדל להפריד ביניהם, אך עשוי לשבץ יחד אם אין ברירה, וההפרה תופיע כאזהרה בלוח האזהרות. <em>שים לב</em>: כדי שהאילוץ יחול על תבנית מסוימת, צריך להפעיל בה את המתג <strong>"אי-התאמה"</strong> (ראה לשונית כללי משימות).',
      bodyFallback:
...
const TASK_RULES_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      placement: 'bottom',
      title: 'מוכנות לשיבוץ',
      body: 'תג זה מסכם את הבדיקה המקדימה הכוללת — <strong>קריטי</strong> אדום / <strong>אזהרה</strong> כתום / <strong>בסדר</strong> ירוק. לחיצה על התג פותחת רשימה מפורטת של כל הממצאים.<br><br><strong>ממצאים קריטיים (חוסמים יצירת שבצ"ק):</strong><br>• <strong>פער כישורים</strong> — אין מספיק משתתפים ברמה/הסמכה הנדרשות לאיוש כל המשבצות של תבנית. פתרון: הוסף משתתפים, הרחב רמות מותרות, או הסר משבצת.<br>• <strong>פער ביום ספציפי</strong> — כוח אדם כולל מספיק, אך ביום (ושעה) ספציפי אין מי שכשיר וזמין יחד — לרוב בגלל אי-זמינות מרוכזת.<br>• <strong>קיבולת חרגה</strong> — סך השעות הנדרשות מהצוות גדול מסך הזמינות (מעל 100% ניצול).<br><br><strong>אזהרות (יצירה אפשרית, מומלץ לבדוק):</strong><br>• <strong>נדירות כשירות</strong> — משבצת עם מועמד יחיד, צפיפות 90–100%, או פער רוטציה במשימת "קבוצה אחידה".<br><br>בנוסף, כל כרטיסיית תבנית מציגה תג <strong>!</strong> או <strong>⚠</strong> ספציפי לתבנית עצמה — לחיצה עליו פותחת רשימה ממוקדת לאותה תבנית.',
      bodyFallback:
        'בראש הלשונית יופיע תג מוכנות — אדום / כתום / ירוק — שמסכם את הבדיקה המקדימה. ממצאים קריטיים (פער כישורים, פער ביום ספציפי, קיבולת חרגה) חוסמים יצירה. אזהרות (נדירות כשירות, צפיפות גבוהה, פערי רוטציה) מאפשרות יצירה אך מומלץ לבדוק.',
...
const SCHEDULE_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      placement: 'bottom',
      title: 'ביטול ושחזור',
      body: 'כפתורי ↪ <strong>ביטול</strong> ו-↩ <strong>שחזור</strong> בסרגל הכותרת מעקבים אחר כל ההחלפות הידניות, החילוצים, וההזרקות שביצעת. המספר בסוגריים מציג את עומק הסטאק. ↪ <strong>ביטול</strong> מחזיר את השבצ"ק למצבו לפני הפעולה האחרונה — <em>כולל</em> כניסות אי-זמינות שנרשמו יחד איתה. ↩ <strong>שחזור</strong> מחזיר את מה שבוטל. הסטאק אינו נשמר בין גרסאות שמורות — אם אתה עומד לנסות שינוי גדול, שמור גרסה תחילה כנקודת חזרה.',
      bodyFallback:
        'בסרגל הכותרת יופיעו כפתורי ↪ <strong>ביטול</strong> ו-↩ <strong>שחזור</strong> שמעקבים אחר כל ההחלפות והחילוצים. המספר בסוגריים מציג את עומק הסטאק.',
...
const PROFILE_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      body: 'הכפתור <strong>📜 שינוי הסמכה</strong> מטפל בתרחיש שבו הסמכה של המשתתף פוקעת או נשללת באמצע השבצ"ק — למשל אישור חממה שתוקפו תם. לחיצה עליו פותחת בורר: בחר אילו הסמכות אבדו ולאיזו תקופה עתידית. המערכת תאתר את <strong>כל</strong> השיבוצים העתידיים שדורשים את אותן הסמכות, ותציע תוכנית החלפה מאוחדת — באותו ממשק של Future SOS, עם תוכניות מדורגות לבחירה. לאחר ההחלה, רישום השינוי מופיע בכרטיס אי-הזמינות שמתחת וניתן להסרה בלחיצת <strong>הסר</strong>. <em>הכפתור מוצג רק למשתתפים שמחזיקים לפחות הסמכה אחת.</em>',
      bodyFallback:
