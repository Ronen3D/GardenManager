  ...
  steps: [
    ...
    },
    {
      ...
      mobileOverride: {
        target: '.gm-edit-sheet-v2 [data-pe-section="identity"]',
        placement: 'top',
        body: 'הגיליון התחתון מציג את שדות הזיהוי: <strong>שם</strong>, <strong>קבוצה</strong>, ו<strong>דרגה</strong> (L0 / L2 / L3 / L4 — אין L1). הדרגה קובעת באילו משבצות המשתתף יוכל להשתבץ. <strong>הסמכות</strong>, <strong>מקדם עומס</strong>, ו<strong>אי התאמה</strong> מופיעים בקטעים נוספים בהמשך הגיליון.',
      },
    },
    {
      id: 'p-4',
...
const TASK_RULES_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      placement: 'inline-start',
      title: 'אי התאמה',
      body: 'כשמסומן — העדפות <strong>אי התאמה</strong> שהוגדרו בכרטיסי המשתתפים יחולו על משמרות התבנית הזו, והאופטימייזר ישתדל לא לשבץ יחד אנשים שסומנו זה מול זה. זהו <strong>אילוץ רך</strong>: כשאין ברירה הם עשויים להשתבץ יחד, וההפרה תופיע כאזהרה. שים לב: המתג כאן רק מפעיל את ההתחשבות — את הזוגות עצמם מגדירים בכרטיסי המשתתפים. ללא הפעלת המתג כאן, הגדרות אי-ההתאמה אינן משפיעות על התבנית.',
      expandFirstTemplate: true,
    },
    {
      id: 't-6c',
      ...
      placement: 'inline-start',
      title: 'ניתן לפיצול',
      body: 'כשמסומן — <strong>וגם פיצול משמרות מופעל בהגדרות האלגוריתם</strong> — המערכת רשאית לפצל מופע לשתי משמרות שוות לשני אנשים <strong>שונים</strong>: גם כדי לאייש משבצת שאחרת תישאר ריקה, וגם כשהפיצול <strong>משפר משמעותית</strong> את האיכות (עונש הפיצול קובע את הסף), ומאחה בחזרה כשכבר לא משתלם. בלתי תלוי ב"חוסם רצף"; ברירת המחדל כבויה — וכל עוד הוא כבוי השיבוץ מתנהג בדיוק כמו היום.',
      expandFirstTemplate: true,
    },
    {
      id: 't-7',
    ...
    },
    {
      ...
      body: 'נמצא תחת המקטע <strong>מתקדם</strong> בכרטיסיית התבנית. מנגנון נפרד מכללי מרווח: בוחרים אילו <strong>משמרות-טריגר</strong> בתבנית מפעילות חלון התאוששות (לפי מספר המשמרת: 1, 2, 3...), ואז <strong>שעות התאוששות</strong> שבהן לא תשובץ שום משימה אחרת בעלת עומס. לדוגמה: סיום משמרת לילה (משמרת 3) → 12 שעות בלי שיבוץ נוסף. אילוץ <strong>קשה</strong> — אם מופר, השבצ"ק פסול.',
      expandFirstTemplate: true,
      expandAdvanced: true,
    },
    {
    ...
    },
    {
      ...
      body: 'נמצא תחת המקטע <strong>מתקדם</strong> בכרטיסיית התבנית. מאפשרים להגדיר ש<em>חלק מהמשמרת</em> נחשב יותר עומס — למשל החלק 06:00–08:00 שוקל ×0.8, או החצי הראשון של משמרת לילה. בכל חלון: טווח שעות, משקל (0–1), ו<strong>חוסם בקצה</strong> — אם מסומן, חלון שנוגע בקצה המשמרת חוסם רצף עם משמרת סמוכה (אילוץ ברמת חלון, נפרד מ"חוסמת רצף" שברמת התבנית).',
      expandFirstTemplate: true,
      expandAdvanced: true,
    },
    {
...
const SCHEDULE_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      placement: 'bottom',
      title: 'לוח מדדים',
      body: '✓ "ישים" — כל התנאים המחייבים עברו. ✗ "לא ישים" — יש הפרה שחובה לתקן. הציון הנומרי (גבוה יותר = שבצ"ק איכותי יותר) משקלל תגמולים (מנוחה ופיזורה) פחות קנסות מתנאים רכים. לחץ על תא האזהרות הכתום לקפיצה ישירה לחלונית ההפרות.',
    },
    {
      id: 's-4',
      target: '.day-nav-wrap',
    ...
    },
    {
      ...
      mobileOverride: {
        target: '.schedule-grid-container .participant-hover[data-assignment-id]:not([data-frozen])',
        title: 'החלפה ידנית — חלק 1',
        body: 'במגע, הקשה על שם משובץ בטבלה פותחת <strong>גיליון תחתון</strong> עם פרטי המשתתף — דרגה, קבוצה, הסמכות, פק"לים ופילוח עומס — ובתחתיתו כפתורי פעולה מהירה <strong>⇄ החלפה</strong> ו-<strong>🆘 חילוץ</strong>. <em>פתח את הגיליון על שם כלשהו</em>; בשלב הבא נסביר את כפתור ההחלפה.',
      },
    },
    {
      id: 's-8-action',
    ...
    },
    {
      ...
      target: '.schedule-grid-container .participant-hover[data-assignment-id]:not([data-frozen])',
      placement: 'top',
      title: 'חילוץ מהגיליון התחתון',
      body: 'הכפתור <strong>🆘 חילוץ</strong> זמין תמיד בגיליון התחתון של משתתף משובץ. אם <strong>מצב חי</strong> אינו פעיל, ייפתח חלון "הפעלת מצב חי" שיבקש לקבוע את הזמן הנוכחי — לאחר אישור, מצב חי יופעל באופן אוטומטי ובורר החילוץ ייפתח. ביטול סוגר את החלון ללא שינוי. בדומה לחילוץ ממסך השיבוץ, התוכניות מדורגות לפי איכות, וניתן לסמן את המוחלף כלא-זמין עם סיבה.',
      mobileOverride: {
        target: '.schedule-grid-container .participant-hover[data-assignment-id]:not([data-frozen])',
        placement: 'top',
        body: 'הקש <strong>🆘 חילוץ</strong> בגיליון של משתתף משובץ. אם מצב חי כבוי, יפתח חלון "הפעלת מצב חי" — קבע יום ושעה ואשר; מצב חי יופעל ובורר החילוץ ייפתח אוטומטית.',
      },
    },
    {
      id: 's-8b',
...
const ALGORITHM_TRACK: TutorialTrack = {
  ...
  steps: [
    ...
    },
    {
      ...
      openAccordion: ['acc-algorithm', 'acc-general'],
      mobileOverride: {
        body: 'מכייל את <strong>המשקלות הרלוונטיות</strong> לנתונים שלך (משתתפים, משימות, הסמכות) ומדלג על אלה שאינם רלוונטיים. אורך מספר דקות, ורק <strong>מציע</strong> — ההגדרות לא ישונו עד שתאשר <strong>החל הגדרות מומלצות</strong>. עדיף על כוונון ידני של הסליידרים.',
      },
    },
    {
      id: 'a-3d',
      target: '.algo-weight-card[data-card="splitting-mode"]',
      placement: 'inline-start',
      title: 'פיצול משמרות — מצב הפעולה להרצה הבאה',
      body: 'בחר אחד משלושה מצבים <strong>להרצה הבאה</strong>: <strong>כבוי</strong> — אף משמרת לא תפוצל; כל משמרת מאוישת על־ידי אדם אחד. <strong>השלמת איוש בלבד</strong> — פיצול יתבצע רק כשבלעדיו תישאר משבצת ריקה; ברגע שהפיצול כבר אינו נחוץ, המערכת מאחה אותו בחזרה. <strong>השלמת איוש ושיפור איכות</strong> — פיצול יתבצע גם כדי להשלים איוש וגם כדי לשפר הוגנות, מנוחה ואיזון; עונש הפיצול קובע מתי הוא נחשב משתלם. ההגדרה <strong>נקפאת על השבצ"ק שייווצר</strong> — שינוי כאן ישפיע רק על השבצ"ק הבא.',
      openAccordion: ['acc-algorithm', 'acc-general'],
    },
    {
      id: 'a-4',
    ...
    },
    {
      ...
      body: 'במצב <strong>"השלמת איוש ושיפור איכות"</strong>, סליידר זה קובע כמה "יקר" נחשב כל פיצול — וזהו <strong>הסף שקובע אם פיצול משתלם</strong>: המערכת תפצל משמרת שניתן לאייש בשלמותה רק כאשר השיפור בהוגנות, במנוחה או באיזון <strong>עולה על הקנס</strong>, ותאחה פיצול בחזרה כשהוא כבר אינו מצדיק אותו. ערך גבוה יותר = פיצול נדיר יותר. פיצול שמונע משבצת ריקה משתלם תמיד ואינו מורתע. הקנס נכלל בכיול האוטומטי.',
      openAccordion: ['acc-algorithm', 'acc-weights'],
    },
    {
      id: 'a-5',
