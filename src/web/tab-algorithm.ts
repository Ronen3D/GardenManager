...
const WEIGHT_GROUPS: WeightGroup[] = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        detail:
          'לסגל יש פחות משימות מתאימות, ולכן טווח העומס שלהם מצומצם יותר. ברירת המחדל ({default}) נמוכה בכוונה בהשוואה למשקל שיוויון העומס הכללי. הגדל אם אתה מבחין שעומס הסגל הופך לא מאוזן.',
      },
      {
        ...
        detail:
          'שני מדדים משולבים, שניהם מול יעד פרופורציונלי לזמינות: (1) החלוקה היומית של כל משתתף צריכה להיות פרופורציונלית לזמינות שלו ביום (מי שזמין 4 שעות ביום אחד ו-24 בשני, צפוי לקבל פי 6 יותר עומס ביום השני, לא חצי-חצי), ו-(2) סך העומס של כל המשתתפים ביום נתון צריך להיות פרופורציונלי לזמינות הקבוצתית באותו יום (יום עם זמינות צוות נמוכה יותר, כמו ערב שבת, צפוי להיות עמוס פחות). ברירת מחדל 144 הופכת את האיזון היומי לאחד מגורמי הניקוד החזקים ביותר.',
      },
    ],
  },
  {
    title: 'מנוחה בין משימות',
    description: 'עד כמה חשוב לשמור על הפסקה בין משימות חוסמות עוקבות. משימות קלות (עומס 0) לא נכללות בחישוב הזה.',
    fields: [
      {
        key: 'minRestWeight',
        ...
        detail:
          'רק פערים שבהם לשתי המשימות הצמודות מופעל "חוסם רצף" (HC-12) נספרים. מעבר בין שתי משימות חוסמות נספר; מעבר שאחד מצדדיו משימה קלה — לא. הנוסחה: minRestWeight × שעות מנוחה מינימליות. בברירת המחדל ({default}), כל שעת מנוחה נוספת = {default} נקודות.',
      },
      {
        ...
        step: 1,
        description:
          'תוספת רכה שלוקחת בחשבון את כל פערי המנוחה — לא רק את הפער הקצר ביותר. עוזרת לאופטימייזר להבחין בין סבבים אחידים לסבבים שבהם רק חלק מהמשתתפים נחים היטב. ערך 0 משבית את התוספת.',
        detail:
          'הנוסחה: restPerGapWeight × Σ √(שעות פער), לכל פער של כל משתתף. הפונקציה √ נותנת תשואה פוחתת — הקטנת פער קצר משפיעה הרבה יותר מהארכת פער שכבר ארוך. בלי תוספת זו, האופטימייזר רואה רק את הפער המינימלי הגלובלי, ולכן לא מצליח לשפר חלוקות סימטריות שבהן כולם תקועים על אותו פער קצר.',
      },
    ],
  },
  {
...
const HC_DESCRIPTIONS: Record<HardConstraintCode, string> = {
  ...
  'HC-14': 'נדרשת הפסקה מינימלית (לפי משך כל חוק מנוחה) בין משימות מאותה קטגוריה לאותו משתתף.',
  'HC-15': 'כשמשתתף מסיים משמרת שהוגדרה כמפעילת התאוששות, הוא לא ישובץ למשימה עם עומס במהלך חלון ההתאוששות שלאחריה.',
  'HC-16': 'שני החצאים של אותה משבצת מפוצלת חייבים להיות מאוישים בשני משתתפים שונים.',
};
