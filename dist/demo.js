...
console.log();
// Stage 1: Setup
const engine = new index_1.SchedulingEngine({
    maxIterations: 2000,
...
console.log(`  Bonus:        ${schedule.score.totalBonus.toFixed(1)}`);
console.log(`  Composite:    ${schedule.score.compositeScore.toFixed(1)}`);
console.log();
// Violations
const hardViolations = schedule.violations.filter((v) => v.severity === index_1.ViolationSeverity.Error);
const warnings = schedule.violations.filter((v) => v.severity === index_1.ViolationSeverity.Warning);
...
// Task Summary
console.log((0, gantt_bridge_1.buildTaskSummary)(schedule));
console.log();
// Rest Profiles
...
console.log((0, gantt_bridge_1.ganttToAscii)(ganttData, 100));
console.log();
// Demo: Manual Swap
...
if (schedule.assignments.length >= 2) {
    ...
    const swapCandidate = schedule.participants.find((p) => p.id !== firstAssignment.participantId && p.group === originalParticipant?.group);
